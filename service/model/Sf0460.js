...
module.exports.insert = async (Sf0460 = {}) => {
  ...
  Sf0460.cd_obs = parseInt(nProx_Codigo) + 1;
	...
	let sql = `insert into (cd_obs, id_empresa, dt_inicial, dt_movimento, id_usuario, ds_obs) 
                 values (:cd_obs, :id_empresa, :dt_inicial, :dt_movimento, :id_usuario, :ds_obs)
						`;
	try {
		await Oracle.insert(sql, Sf0460);
    return Sf0460.cd_obs;
	} catch (err) {
		throw new Error(err);
	}
}